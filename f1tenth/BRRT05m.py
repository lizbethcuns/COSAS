...
def apply_spline_and_resample(points, separation_distance=0.5, smoothing=10.0):
    """
...
if __name__ == "__main__":
    ...
    if not path_indices:
        ...
        # Tarea pide: Separación 0.5 metros
        print("Generando curva suavizada (0.5m)...")
        # smoothing=5.0 es un valor medio, auméntalo si la curva vibra mucho
        path_smooth_05 = apply_spline_and_resample(centered_path, separation_distance=0.5, smoothing=5.0)
        save_path_as_csv(path_smooth_05, "trayectoria_suavizada_0.5m.csv")
        ...
        plt.plot(smooth_px[:,0], smooth_px[:,1], 'g-', linewidth=2, label='Spline Final (0.5m)')
        plt.legend()
